...
vec3 blurDownsampleBrightness(sampler2D tex, vec2 uv, vec2 texelSize, float threshold, float knee) {
  return brightnessPassT(tex, uv, threshold, knee) / 2.0 +
         brightnessPassT(tex, uv - uRadius * texelSize.xy, threshold, knee) / 8.0 +
         brightnessPassT(tex, uv + uRadius * texelSize.xy, threshold, knee) / 8.0 +
         brightnessPassT(tex, uv + uRadius * vec2(texelSize.x, -texelSize.y), threshold, knee) / 8.0 +
         brightnessPassT(tex, uv - uRadius * vec2(texelSize.x, -texelSize.y), threshold, knee) / 8.0;
}
